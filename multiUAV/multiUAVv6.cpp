...
int main()
{
	...
	config >> temp >> temp >> temp;
	int uavPos[5];
	if (temp == "LU") for (int i = 0; i < 5; i++) uavPos[i] = 1;
	else if (temp == "LD") for (int i = 0; i < 5; i++) uavPos[i] = 2;
	else if (temp == "RU") for (int i = 0; i < 5; i++) uavPos[i] = 3;
	else for (int i = 0; i < 5; i++) uavPos[i] = 4;
	point curPos[5];
	config >> temp >> temp >> temp;
	nUAV = stoi(temp);
	...
	while (repeat <= nClusters)
	{
		...
		{
			point pos = findUavPos(uavPos[i]);
			curPos[i].x = pos.x; curPos[i].y = pos.y;
			cout << "curPos = " << curPos[i].x << ", " << curPos[i].y << endl;
		}
		...
		*/
		...
		/*if (mode == 1)
		{
			...
			else
			{
				...
				{
					//if (episod % 100 == 0) cout << "Episod = " << episod << endl;
					uavPos[episod] = curPos[0];
					int action[SENSOR];
					int prev_action[SENSOR] = { 0 };
					AgeVector max_rew;
					//if (!bPathExist)
					//	printf("Try to find best action ..\n");
					for (int i = 0; !bPathExist && i < nState; i++)
					{
						...
						{
							//printf("\n<<\n");
							//printAction(i, nUAV, rewAge, max_rew, curPos, curAOI);
							iBestAction = i;
							max_rew = rewAge;
						}
					...
					//for (int j = 0; j < nUAV; j++)
					//{
					//	action[j] = (iBestAction >> (2 * j)) & 3;
					//	prev_action[j] = action[j];
					//}
					AgeVector rewAge = Q.Reward(curAOI, curPos[0], action[0], prev_action[0], false, &uPath);
					if (bPathExist == false)
					{
						//printf("episod = %d : Finding new Path ...\n", episod);
						nMeet = 0;
					...
					/*printf("Reward = (%s)\n", rewAge.getstr(tmp));
					printf("AOI = (%s) = %2.2f  ", curAOI.getstr(tmp), max_rew.sum());
					for (int j = 0; j < SENSOR; j++)
					printf("S[%d]=(%2d, %2d) ", j, Sensors[j].x, Sensors[j].y);
					
					for (int i = 0; i < nSensors; i++)
					{
						int iUAV = rewAge.indexUAV;
						/*
						if (iUAV >= 0)
						{

						int d = curPos[iUAV].distance(Sensors[i]);
						printf("\nEpisod=%d : UAV-%2d=>S-%2d [dist=%d]: Pu=(%2d,%2d), Ps=(%2d,%2d), aoi=%2.2f, act=<%2d,%2d> rew=(%2.2f)",
						episod, iUAV, i, d, curPos[iUAV].x, curPos[iUAV].y, Sensors[i].x, Sensors[i].y,
						curAOI.a[i], Action[action[iUAV]].x, Action[action[iUAV]].y, rewAge.a[i]);
						} 
					}
					//if (episod % 1000 == 0)
					//printf("\nEpisod = %d => ...\n", episod);
					...
					for (int i = 0; i < nSensors; i++)
					{
						for (int j = 0; j < nUAV; j++)
						{
							if (curPos[j] == Sensors[i])
							{
								curAOI.a[i] = 0;
								nMeet++;
								//printf("Episod = %d Meeting S[%d] By UAV-%d at Location (%d, %d)... ", 
								//	episod, i, j, Sensors[i].x, Sensors[i].y);
								//printf("(%d/%d, dist=%d)\n", nMeet, res.nTotal[0], res.dist[0]);
								uPath.insert(uPath.begin(), CheckPoint(episod, i, curPos[j]));
								bMeet = true;
								//getchar();
								char tmp[4000];
								//printf("aoi = %s ", curAOI.getstr(tmp));
							}
						}
					}
				...
				for (int i = minCycle.slot_begin; i <= minCycle.slot_end; i++)
				{
					...
					//for (int j = 0; j < nSensors; j++) if (uavPos[i] == Sensors[j]) tours << j + 1 << endl;
				}
				tours << -1 << endl << "EOF";
				tours.close();
				route.close();
		...
		if (mode == 2)
		{
			...
			*/
			...
			for (int rnd = 0; rnd < RNDMAX; rnd++)
			{
				...
				{
					curAOI = curAOI + one;
					for (int m = 0; m < nUAV; m++) curPos[m] = UAV_pos_opt[(i + (m * (u / nUAV))) % u];
					for (int j = 0; j < nUAV; j++)
					{
						for (int m = 0; m < v; m++)
						{
							if (curPos[j] == Sensors_opt[m]) curAOI.a[m] = 0;
						}
					}
